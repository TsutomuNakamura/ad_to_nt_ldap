...
class ModAdapTest < Minitest::Test
  ...
  def test_add_user_should_failed_if_the_argument_of_password_was_nil
    adap = Adap.new({
      :ad_host   => "localhost",
      :ad_binddn => "CN=Administrator,CN=Users,DC=mysite,DC=example,DC=com",
      :ad_basedn => "CN=Users,DC=mysite,DC=example,DC=com",
  ...
  def test_add_user_should_failed_if_the_argument_of_password_was_empty_string
    adap = Adap.new({
      :ad_host   => "localhost",
      :ad_binddn => "CN=Administrator,CN=Users,DC=mysite,DC=example,DC=com",
      :ad_basedn => "CN=Users,DC=mysite,DC=example,DC=com",
    ...
    exception = assert_raises RuntimeError do
      #result = adap.get_password("foo")
      adap.add_user("uid=foo,ou=Users,dc=mysite,dc=example,dc=com", {:objectclass => ["top", "person"], :cn => "foo"}, "")
    end
    assert_equal(
      exception.message,
      'add_user() requires password. Set a hashed password of the user foo please.'
  ...
  def test_add_user_should_failed_if_ldap_add_was_failed
    ...
    mock_ldap_get_operation_result.expects(:code).returns(1).times(1)
    ...
    assert_equal({
      :code => 1,
      :operations => [:add_user],
      :message => "Failed to add a user uid=foo,ou=Users,dc=mysite,dc=example,dc=com in add_user() - Some error"
    }, ret)
  end
